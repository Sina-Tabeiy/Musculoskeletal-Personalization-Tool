...
% ----------------------------------------------------------------------- %
function osimModel = applyTorsionToMuscleAttachments(osimModel, CORA, aSegmentName, deformity_angle)
    ...
    for n_p = 0:N_p-1
        ...
        if strcmp(attachBodyName, aSegmentName)
            ...
            if strcmp(curr_pathpoint_class, 'PathPoint') || strcmp(curr_pathpoint_class, 'ConditionalPathPoint')
                ...
                if  musAttachLocCoords(2) < CORA(2)
          
                % New points and axis
                deviation_angle = (abs(musAttachLocCoords(2) - CORA(2)) * deformity_angle) /abs(musAttachLocCoords(2));
                TorsRotMat = rotx(deviation_angle);
                new_musAttachLocCoords = (TorsRotMat*musAttachLocCoords')';

                else

                new_musAttachLocCoords = musAttachLocCoords;
